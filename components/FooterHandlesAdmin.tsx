...
type FooterHandlesAdminProps = {
  readOnly?: boolean;
};

export default function FooterHandlesAdmin({
  readOnly = false,
}: FooterHandlesAdminProps) {
    ...
    <div className="mt-10 grid gap-8 lg:grid-cols-[minmax(280px,420px)_1fr]">
      {!readOnly ? (
        ...
        <div className="scrap-card p-6 text-sm text-[#17001C]/70">
          <h2 className="text-lg text-[#17001C]">Footer profiles read-only</h2>
          <p className="mt-3">
            Footer profiles can be viewed here. Adding, editing, removing, and
            profile picture uploads are locked.
          </p>
          {message ? (
            <p className="mt-3 text-sm text-[#17001C]/70">{message}</p>
          ) : null}
